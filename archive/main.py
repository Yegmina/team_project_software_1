...
class Game:
    ...
    def make_choice(self):
        ...
        # Filter out choices that have already been made, to avoid showing them again
...
##----------------- Game starts here ------------------ ##
        ...
        while game.game_over == False :
            ...
            game.id = db.run(f"SELECT id FROM saved_games WHERE input_name = '{game.name}';")[0][0]
            game.infected_country = db.run(f"SELECT COUNT(*) FROM airport_info "
                                           f"WHERE infected = 1 "
                                           f"AND game_id = {game.id};")[0][0]
            ##------ Changing game variables based on themselves, others and random
